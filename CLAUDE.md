...
- **Icons:** `node scripts/make-icons.mjs` redraws `icons/icon-{16,48,128}.png` (the orb on `--bg`) with no dependencies. Rerun it if the orb colours change.
- **Web Store package:** `scripts/package.sh` zips only the runtime files into `dist/still-<version>.zip`. Add any new runtime file to its list. Pushing a tag `v<version>` runs `.github/workflows/release.yml`, which fails unless the tag matches `manifest.json`'s version, then runs the same script and attaches the zip to a GitHub Release. `CHROMEWEBSTORE.md` holds the listing copy, privacy answers and version history. Update it on user-facing or `manifest.json` changes, and bump `version` for each upload.
...
- **Two pages, shared globals.** `newtab.html` loads `common.js` then `app.js`; `favourites.html` loads `common.js` then `favourites.js`. `common.js` defines the globals both pages use: `store` (try/catch-wrapped localStorage JSON), `today()` (local-time `YYYY-MM-DD`), `loadThoughts()`, the favourites/journal helpers and `shortDate()`. Keep scripts as non-module classic scripts and keep load order. Each page starts with an `async function init()` (async/await, not `.then()` chains).
- **All state is localStorage**, shared by both pages (same extension origin). This is deliberate, even though the chrome-extensions skill prefers `chrome.storage`: there's no service worker, it needs no `storage` permission, the pages still work as a static website, and the journal can flush synchronously on `pagehide`. Only migrate for a concrete reason such as cross-device sync.
